...
static u8* fixedHeapPtr = NULL;
static u8* fixedHeapEnd = NULL;
...
void m3_ResetHeap (void)
{
    fixedHeapPtr  = fixedHeap;
    fixedHeapEnd  = fixedHeap + d_m3FixedHeap;
    fixedHeapLast = NULL;
}

void *  m3_Malloc_Impl  (size_t i_size)
{
    if (!fixedHeapPtr) { fixedHeapPtr = fixedHeap; fixedHeapEnd = fixedHeap + d_m3FixedHeap; }
    u8 * ptr = fixedHeapPtr;
